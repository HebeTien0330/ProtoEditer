...
:@LastEditTime: 2025/1/20 16:15:34
:Description: 
...
class CustomScript(CustomBase):
    """
    Gen proto file for go
    """
    def __init__(self, targetDir):
        super().__init__(targetDir)
        finalPath =  f"{self.m_targetDir}\\genForGo"
...
""" % (filename.split(".")[0])
            for protocol, data in protos.items():
                self.log(f"create {filename} -> {protocol}")
                message = ""
...
""" % payload
    ...
    def genCode(self):
        ...
        if curOs == "Windows":
            command = f"{os.getcwd()}\\tools\\protoc.exe --go_out={codeDir} --proto_path={protoDir} {protoDir}\\*.proto"
            print(command)
            subprocess.run(['powershell', '-Command', command], capture_output=True, text=True)
            return
        if curOs == "Linux":
            command = f"{os.getcwd()}/tools/protoc --go_out={codeDir} --proto_path={protoDir} {protoDir}/*.proto"
            subprocess.run([command], capture_output=True, text=True)
        self.log("create code finish...")

    def run(self, protosData):
        self.log("start gen proto file for go...")
